...
ALL frontend work (React, Next.js, Vite, TypeScript, UI, routes, design systems)
MUST follow the senior-React standard, even without invoking a skill.

- **Law file**: `.claude/skills/frontend-agent/references/react-senior-standard.md` — read before writing/reviewing any frontend code.
- **Stack**: Next.js 16 App Router *or* React 19 + Vite 8 · TS 5 strict (no `any`, `type` not `interface`) · TanStack Router in Vite (`react-router-dom` FORBIDDEN) · TanStack Query v5 · Zustand · Tailwind v4 (no CSS/inline) · React Hook Form + Zod · Vitest/RTL/Playwright · **pnpm only** · ESLint v9 + Prettier (no semicolons, single quotes).
...
- **Always**: 4 UI states (Loading/Empty/Error+retry/Data) · test-first in Write mode · async cleanup · accessible by default · Conventional Commits.

Route frontend tasks through the `frontend-agent` skill (or `frontend-dev`/`coder`).

### Skill / Plugin / Agent Connectivity Map

Everything is wired — pick the entry point, it pulls in the rest.

| Domain | Entry skill | Chains into |
|--------|-------------|-------------|
| **Frontend** | `frontend-agent` | `react-senior-standard.md` (law) → `ui-ux-pro-max`, `frontend-design`, `frontend-error-ux`, `design-system-steward`, `react-19-frontend-agent`, `react-19-patterns`, `nextjs-app-router-practices`, `typescript-react-routing` + `agent-assets/` |
...
| **Bootstrap** | `best-powers` | copies CLAUDE.md + AGENTS.md + `.mcp.json` + all `.claude/skills` + `.claude/agents` + `agent-assets/` into any folder |
...
| **Review / quality** | `code-reviewer` | `github-code-review`, `verification-quality`, `pair-programming`, `review-pr` |
| **GitHub** | `github-workflow-automation` | `github-project-management`, `github-multi-repo`, `github-release-management` |
| **Docs / memory** | `project-documentation-wiki` | `docs/wiki/*`, nearby `FEATURE.md`, `design.md`, `sidecar-docs` |

Connection rules:
- The router skill for a domain is the single entry point; load deeper skills/references only as the task needs them.
- `best-powers` is the propagation mechanism — anything added to this workspace's `.claude/skills`, `.claude/agents`, or `agent-assets/` ships to every new folder it bootstraps. Keep new domain rules there.
- Skills coordinate, MCP tools execute coordination, the Agent tool executes work.
